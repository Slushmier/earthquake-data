...
global_grid <- st_make_grid(sfc_global, 5, crs = st_crs(sf_df), square = F)
...
earthquake_hexsum <- earthquakes %>% group_by(hexid) %>% 
  summarize(events = n(), Deaths = sum(Deaths, na.rm = T)) %>% 
...
threat level to consulates within counties where the State Department may 
...
geographic coordinates, the magnitude, and the focal depth
...
is limited time to discuss the results and the methods that went into them.

I chose to do my analysis in R rather than a traditional geospatial tool 
like ArcGIS or QGIS in order to ensure that any team mates would be able to
easily access and review the analysis.
