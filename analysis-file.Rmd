...
sf_earthquakes <- st_sf()

sfc_global <- st_sfc(st_polygon(list(rbind(c(-180,-90), c(-180, 90), 
                                           c(180, 90), c(180, -90),
                                           c(-180, -90)))))
...
hexid <- sapply(st_intersects(x = so_acled, y = so_grid), 
                function(z) if (length(z)==0) NA_integer_ else z[1])
so_acled$hexid <- hexid
so_hexsum <- so_acled %>% group_by(hexid) %>% 
  summarize(events = n(), fatalities = sum(fatalities)) %>% 
  st_set_geometry(NULL)
so_grid <- left_join(so_grid, so_hexsum) %>% 
